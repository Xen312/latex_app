...
export default function PdfViewer({ pdfUrl }: PdfViewerProps) {
  ...
  async function loadPdf() {
    ...
    try {
      ...
      const fitScale = parseFloat(((containerWidth - 32) / viewport.width).toFixed(2));
      setScale(fitScale);
    } catch (err) {
  ...
  function scrollToCenter(newScale: number, oldScale: number) {
  ...
  function resetZoom() {
    ...
    pdfRef.current.getPage(1).then((page: any) => {
      const viewport = page.getViewport({ scale: 1 });
      const containerWidth = containerRef.current?.clientWidth || 800;
      setScale(parseFloat(((containerWidth - 32) / viewport.width).toFixed(2)));
    });
  }
  ...
  function onTouchMove(e: React.TouchEvent) {
    ...
    if (lastDist.current !== null) {
      ...
      setScale(s => parseFloat(Math.min(Math.max(s * ratio, 0.5), 3.0).toFixed(2)));
    }
